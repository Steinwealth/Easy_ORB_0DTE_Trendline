...
class PrimeUnifiedTradeManager:
    ...
    def _initialize_etrade_trading(self):
        ...
        try:
            ...
            # Account selection is handled separately based on ETRADE_MODE
            etrade_mode = get_config_value('ETRADE_MODE', 'demo')
            trader_env = 'prod'  # Always use production tokens
            
            log.info(f"🔧 Initializing E*TRADE trading: Mode={etrade_mode}, Environment=prod (sandbox phased out)")
            log.info(f"   Demo mode: Will select sim account")
            log.info(f"   Live mode: Will select live account")
            
            # Initialize E*TRADE trading with production environment
            self.etrade_trading = PrimeETradeTrading(environment=trader_env)
    ...
    async def _process_live_signal(self, signal: PrimeSignal, market_data: Dict[str, Any]) -> TradeResult:
        ...
        try:
            ...
            if self.etrade_trading and self.etrade_trading.is_authenticated():
                try:
                    log.info(f"🚀 Executing E*TRADE order: {signal.symbol} {quantity} shares (smart path respects USE_MARKET_ORDERS / ENABLE_SMART_EXECUTION)")
                    
                    # Rev 00146: Add retry logic for E*TRADE API calls (transient failures)
                    max_retries = 3
                    retry_delay = 2.0
                    order_result = None
                    signal_ts_iso = datetime.now(timezone.utc).isoformat()
                    strat_label = (
                        signal.strategy_mode.value
                        if hasattr(signal.strategy_mode, "value")
                        else str(signal.strategy_mode)
                    )
                    oid_fn = getattr(self.etrade_trading, "_extract_order_id_from_response", None)
                    exec_ctx = {
                        "confidence": float(getattr(signal, "confidence", 0) or 0),
                        "seconds_from_signal": 40.0,
                    }
                    if isinstance(market_data, dict):
                        exec_ctx["breakout_score"] = float(market_data.get("breakout_score", 0) or 0)
                        vr = float(market_data.get("volume_ratio", 0) or 0)
                        if vr > 0:
                            exec_ctx["velocity"] = min(0.01, vr * 0.002)
                    
                    for attempt in range(max_retries):
                        try:
                            order_result = await execute_equity_order_smart(
                                self.etrade_trading,
                                symbol=signal.symbol,
                                quantity=quantity,
                                side="BUY",
                                strategy=f"ORB_SO:{strat_label}",
                                trade_id=position.position_id,
                                signal_ts_iso=signal_ts_iso,
                                reference_price=float(signal.price or 0.0),
                                execution_context=exec_ctx,
                            )
                            oid = oid_fn(order_result) if callable(oid_fn) else None
                            if oid:
                                if not isinstance(order_result, dict):
                                    order_result = {}
                                order_result.setdefault("orderId", oid)
                ...
                if symbol in market_data:
                    ...
                    if stealth_decision.action == "EXIT":
                        ...
                        if self.signal_only_mode or is_simulated:
                            ...
                            # LIVE EXIT: Execute real E*TRADE sell order (urgency-aware smart path)
                            if self.etrade_trading and self.etrade_trading.is_authenticated():
                                try:
                                    er = stealth_decision.exit_reason
                                    er_val = er.value if er is not None else "unknown"
                                    plan = map_equity_exit_urgency(er_val)
                                    log.info(
                                        "💸 Executing E*TRADE sell: %s qty=%s urgency=%s style=%s",
                                        symbol,
                                        position.quantity,
                                        plan.exit_urgency,
                                        plan.exit_execution_style,
                                    )
                                    sell_order_result = await execute_equity_order_smart(
                                        self.etrade_trading,
                                        symbol=symbol,
                                        quantity=position.quantity,
                                        side="SELL",
                                        strategy="ORB_SO:stealth_exit",
                                        trade_id=str(getattr(position, "position_id", "") or f"{symbol}_exit"),
                                        signal_ts_iso=datetime.now(timezone.utc).isoformat(),
                                        reference_price=float(exit_price or 0.0),
                                        exit_reason=er_val,
                                        exit_plan=plan,
                                    )
                                    oid_fn = getattr(self.etrade_trading, "_extract_order_id_from_response", None)
                                    oid = oid_fn(sell_order_result) if callable(oid_fn) else None
                                    if oid and isinstance(sell_order_result, dict):
                                        sell_order_result.setdefault("orderId", oid)
...
🎯 <b>Signal-Only Mode:</b> This validates the system would have closed this position at ${exit_price:.2f} in Live Mode
    ...
    def _exit_reason_from_string(self, exit_reason_str: str) -> ExitReason:
        """Map prime_trading_system string reasons to ExitReason (live health / EOD)."""
        mp = {
            "EMERGENCY_BAD_DAY_DETECTED": ExitReason.RAPID_EXIT,
            "WEAK_DAY_EARLY_EXIT": ExitReason.TIME_EXIT,
            "RED_DAY_POST_EXECUTION_EXIT": ExitReason.RAPID_EXIT,
            "End of Day Close": ExitReason.END_OF_DAY_CLOSE,
        }
        return mp.get(exit_reason_str, ExitReason.END_OF_DAY_CLOSE)
    
    def _resolve_active_symbol_key(self, symbol: str) -> Optional[str]:
    ...
    async def close_position(self, symbol: str, exit_reason_str: str) -> TradeResult:
        """
        Public close entry used by prime_trading_system (live EOD stragglers, portfolio health).
        Prefers the unified in-memory book; if absent, sells a broker-held long from portfolio snapshot.
        """
        er = self._exit_reason_from_string(exit_reason_str)
        resolved = self._resolve_active_symbol_key(symbol)
        exit_price = 0.0
        if resolved:
            pos = self.active_positions.get(resolved)
            if pos:
                exit_price = float(getattr(pos, "current_price", 0) or getattr(pos, "entry_price", 0) or 0.0)
        qsym = (resolved or symbol.strip() or "").strip()
        if self.etrade_trading and self.etrade_trading.is_authenticated() and qsym:
            try:
                quotes = await asyncio.to_thread(self.etrade_trading.get_quotes, [qsym])
                if quotes:
                    exit_price = float(getattr(quotes[0], "last_price", 0) or exit_price or 0.0)
            except Exception as qe:
                log.warning("close_position: quote fetch failed for %s: %s", symbol, qe)
        if exit_price <= 0:
            if resolved and self.active_positions.get(resolved):
                exit_price = float(self.active_positions[resolved].entry_price or 0.01)
            else:
                exit_price = 0.01
        if resolved:
            return await self._close_position(resolved, er, exit_price)
        if not self.etrade_trading or not self.etrade_trading.is_authenticated():
            log.error("close_position(%s): no E*TRADE session", symbol)
            return TradeResult(action=TradeAction.HOLD, symbol=symbol, reasoning="No E*TRADE session")
        try:
            port = await asyncio.to_thread(self.etrade_trading.get_portfolio)
        except Exception as pe:
            log.error("close_position(%s): portfolio failed: %s", symbol, pe)
            return TradeResult(action=TradeAction.HOLD, symbol=symbol, reasoning="Portfolio fetch failed")
        target = symbol.strip().upper()
        et_sym = None
        qty = 0
        total_cost = 0.0
        for p in port or []:
            ps = (getattr(p, "symbol", "") or "").strip().upper()
            if ps == target:
                et_sym = (getattr(p, "symbol", "") or "").strip() or symbol.strip()
                qty = int(getattr(p, "quantity", 0) or 0)
                total_cost = float(getattr(p, "total_cost", 0.0) or 0.0)
                break
        if qty <= 0:
            log.warning("close_position(%s): no broker long in portfolio", symbol)
            return TradeResult(action=TradeAction.HOLD, symbol=symbol, reasoning="No broker position")
        entry_price = (total_cost / qty) if qty else exit_price
        order_result = None
        try:
            plan = map_equity_exit_urgency(er.value)
            order_result = await execute_equity_order_smart(
                self.etrade_trading,
                symbol=et_sym,
                quantity=qty,
                side="SELL",
                strategy="ORB_SO:straggler_close",
                trade_id=f"LIVE_STRAGGLER_{target}",
                signal_ts_iso=datetime.now(timezone.utc).isoformat(),
                reference_price=float(exit_price or 0.0),
                exit_reason=er.value,
                exit_plan=plan,
            )
            oid_fn = getattr(self.etrade_trading, "_extract_order_id_from_response", None)
            oid = oid_fn(order_result) if callable(oid_fn) else None
            if oid and isinstance(order_result, dict):
                order_result.setdefault("orderId", oid)
        except Exception as oe:
            log.error("close_position broker sell failed %s: %s", symbol, oe)
            return TradeResult(action=TradeAction.HOLD, symbol=symbol, reasoning=str(oe))
        if not order_result or "orderId" not in order_result:
            log.error("close_position(%s): sell order missing orderId: %s", symbol, order_result)
        final_pnl = (exit_price - entry_price) * qty
        final_pnl_pct = ((exit_price - entry_price) / entry_price) if entry_price else 0.0
        if self.alert_manager:
            try:
                await self.alert_manager.send_trade_exit_alert(
                    symbol=et_sym or symbol,
                    side="SELL",
                    quantity=qty,
                    entry_price=entry_price,
                    exit_price=exit_price,
                    pnl_dollars=final_pnl,
                    pnl_percent=final_pnl_pct * 100,
                    exit_reason=er.value,
                ...
                log.error("close_position: exit alert failed: %s", ae)
        return TradeResult(
            action=TradeAction.CLOSE,
            symbol=et_sym or symbol,
            price=exit_price,
            exit_reason=er,
            reasoning="Broker-only close (no unified book row)",
        )
    ...
    async def _close_position(self, symbol: str, exit_reason: ExitReason, exit_price: float) -> TradeResult:
        ...
        try:
            ...
            if self.etrade_trading:
                order_result = None
                try:
                    plan = map_equity_exit_urgency(exit_reason.value if exit_reason else "unknown")
                    order_result = await execute_equity_order_smart(
                        self.etrade_trading,
                        symbol=symbol,
                        quantity=position.quantity,
                        side="SELL",
                        strategy="ORB_SO:close_position",
                        trade_id=str(getattr(position, "position_id", "") or symbol),
                        signal_ts_iso=datetime.now(timezone.utc).isoformat(),
                        reference_price=float(exit_price or 0.0),
                        exit_reason=exit_reason.value if exit_reason else "",
                        exit_plan=plan,
                    )
                    oid_fn = getattr(self.etrade_trading, "_extract_order_id_from_response", None)
                    oid = oid_fn(order_result) if callable(oid_fn) else None
            ...
            _pos_meta = getattr(position, "metadata", None) or {}
            if _pos_meta.get("is_so_trade"):
                try:
                    from observability import observability

                    _opened = getattr(position, "entry_time", None)
                    if _opened and getattr(_opened, "tzinfo", None) is None:
                        _opened = _opened.replace(tzinfo=timezone.utc)
                    observability.emit_so_trade_closed(
                        trade_id=str(
                            getattr(position, "position_id", None)
                            or getattr(position, "trade_id", None)
                            or symbol
                        ),
                        symbol=symbol,
                        ...
                        opened_at=_opened,
                        metadata_json={"execution_path": "live_stealth"},
                    )
                except Exception:
                    pass
